...
func HandleCronJobUniverseSystemsTask(ctx context.Context, t *asynq.Task) error {
	...
	defer cancel()
	err = database.Use(db).UniverseSystem.WithContext(dbctx).Save(&record)
	if err != nil {
	...
	for _, planet := range data.Planets {
		...
		for _, asteroidBelt := range planet.AsteroidBelts {
			...
			entryIDBelt, err := queueClient.Enqueue(taskBelt, ESI_UNIVERSE_QUEUE.GetQueue())
			if err != nil {
		...
		for _, moon := range planet.Moons {
			...
			entryIDMoon, err := queueClient.Enqueue(taskMoon, ESI_UNIVERSE_QUEUE.GetQueue())
			if err != nil {
	...
	for _, stargate := range data.Stargates {
		...
		entryIDStargate, err := queueClient.Enqueue(taskStargate, ESI_UNIVERSE_QUEUE.GetQueue())
		if err != nil {
	...
	for _, station := range data.Stations {
		...
		entryIDStation, err := queueClient.Enqueue(taskStation, ESI_UNIVERSE_QUEUE.GetQueue())
		if err != nil {
